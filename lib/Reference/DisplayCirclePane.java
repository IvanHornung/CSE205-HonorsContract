...
public class DisplayCirclePane extends GridPane
{
    ...
    private ArrayList<Circle> tempList, circleList;

    private CanvasPane canvas; // where to draw Circles
    private GridPane ctrlPane;
    private HBox hbox;
    private VBox vbox;
    private ComboBox<String> comboBoxColors = new ComboBox<String>();
    ...
    public DisplayCirclePane()
    {
        ...
        btnErase.setMinWidth(Assignment7.WINSIZE_X/3);
        btnErase.setOnAction(new ButtonHandler()); //action
        btnUndo = new Button("Undo");
        btnUndo.setMinWidth(Assignment7.WINSIZE_X/3);
        btnUndo.setOnAction(new ButtonHandler()); //action
        comboBoxColors = new ComboBox<String>();
        comboBoxColors.setMinWidth(Assignment7.WINSIZE_X/3);
        comboBoxColors.setOnAction(new ColorComboBoxHandler()); //action
        ctrlPane.add(btnErase, 1, 1, 1, 1);
        ctrlPane.add(btnUndo, 2, 1 ,1 ,1);
        ctrlPane.add(comboBoxColors, 3, 1 ,1 ,1);
        comboBoxColors.getItems().addAll("BLACK (default)", "RED", "BLUE", "GREEN", "ORANGE");
        comboBoxColors.setValue("BLACK (default)");
    ...
    {   //instance variables
        ...
        public CanvasPane()
        {
        	...
        	placeholder = new Circle(0, 0, 0); //assuming initial x and y are 0,0 respectively
        	circleColor = Color.BLACK;
        	placeholder.setStroke(circleColor);
